-- Migration 060: Fix admin_create_user — wrap in transaction to prevent orphans
...
begin
  ...
  -- Check for duplicate email in auth.users BEFORE inserting
  if exists (select 1 from auth.users where email = p_email) then
    raise exception 'Un compte avec cet email existe deja: %', p_email;
  end if;

  -- All inserts wrapped in a block — if any fails, PostgreSQL raises exception
  -- which rolls back all side effects in this function
...
exception when others then
  -- If auth.users was created but later steps failed, clean up the orphan
